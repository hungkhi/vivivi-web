...
import type { Background } from '@/lib/supabase/types';
...
export default function BackgroundCard({ background, owned, equipped, index, onClick }: BackgroundCardProps) {
  const isPro = background.tier !== 'free';
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-video rounded-xl overflow-hidden ring-1 ring-white/10 group-hover:ring-[var(--accent-pink)]/40 transition-all duration-300">
        <img
          src={background.thumbnail || background.image}
          alt={background.name}
        ...
        {!owned && background.price_vcoin > 0 && (
          <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-full glass text-xs font-medium">
            ...
            <span className="text-[var(--accent-gold)]">{background.price_vcoin}</span>
          </div>
        )}
